...
import { FuseNavigationItem, FuseVerticalNavigationAppearance, FuseVerticalNavigationMode, FuseVerticalNavigationPosition } from '@tropi/components/navigation/navigation.types';
import { FuseVerticalNavigationAsideItemComponent } from '@tropi/components/navigation/vertical/components/aside/aside.component';
...
import { FuseVerticalNavigationGroupItemComponent } from '@tropi/components/navigation/vertical/components/group/group.component';
...
import { FuseScrollbarDirective } from '@tropi/directives/scrollbar/scrollbar.directive';
...
@Component({
    selector       : 'tropi-vertical-navigation',
    templateUrl    : './vertical.component.html',
    styleUrls      : ['./vertical.component.scss'],
    animations     : tropiAnimations,
    encapsulation  : ViewEncapsulation.None,
    changeDetection: ChangeDetectionStrategy.OnPush,
    exportAs       : 'tropiVerticalNavigation',
    standalone     : true,
...
export class FuseVerticalNavigationComponent implements OnChanges, OnInit, AfterViewInit, OnDestroy
{
    ...
    private _tropiScrollbarDirectives!: QueryList<FuseScrollbarDirective>;
    private _tropiScrollbarDirectivesSubscription: Subscription;
    ...
    @HostBinding('class') get classList(): any
    {
        /* eslint-disable @typescript-eslint/naming-convention */
        return {
            'tropi-vertical-navigation-animations-enabled'             : this._animationsEnabled,
            [`tropi-vertical-navigation-appearance-${this.appearance}`]: true,
            'tropi-vertical-navigation-hover'                          : this._hovered,
            'tropi-vertical-navigation-inner'                          : this.inner,
            'tropi-vertical-navigation-mode-over'                      : this.mode === 'over',
            'tropi-vertical-navigation-mode-side'                      : this.mode === 'side',
            'tropi-vertical-navigation-opened'                         : this.opened,
            'tropi-vertical-navigation-position-left'                  : this.position === 'left',
            'tropi-vertical-navigation-position-right'                 : this.position === 'right',
        };
        /* eslint-enable @typescript-eslint/naming-convention */
    }
    ...
    /**
     * Setter for tropiScrollbarDirectives
     */
    @ViewChildren(FuseScrollbarDirective)
    set tropiScrollbarDirectives(tropiScrollbarDirectives: QueryList<FuseScrollbarDirective>)
    {
        // Store the directives
        this._tropiScrollbarDirectives = tropiScrollbarDirectives;

        // Return if there are no directives
        if ( tropiScrollbarDirectives.length === 0 )
        {
            return;
        }

        // Unsubscribe the previous subscriptions
        if ( this._tropiScrollbarDirectivesSubscription )
        {
            this._tropiScrollbarDirectivesSubscription.unsubscribe();
        }

        // Update the scrollbars on collapsable items' collapse/expand
        this._tropiScrollbarDirectivesSubscription =
            merge(
                ...
                .subscribe(() =>
                {
                    ...
                    tropiScrollbarDirectives.forEach((tropiScrollbarDirective) =>
                    {
                        tropiScrollbarDirective.update();
                    });
                });
    }
    ...
    ngAfterViewInit(): void
    {
        ...
        setTimeout(() =>
        {
            ...
            if ( !this._navigationContentEl.nativeElement.classList.contains('ps') )
            {
                ...
                const activeItem = this._navigationContentEl.nativeElement.querySelector('.tropi-vertical-navigation-item-active');
            ...
            else
            {
                ...
                this._tropiScrollbarDirectives.forEach((tropiScrollbarDirective) =>
                {
                    // Skip if not enabled
                    if ( !tropiScrollbarDirective.isEnabled() )
                    {
                        return;
                    }

                    // Scroll to the active element
                    tropiScrollbarDirective.scrollToElement('.tropi-vertical-navigation-item-active', -120, true);
                });
            }
        });
    }
    ...
    private _showOverlay(): void
    {
        ...
        this._overlay.classList.add('tropi-vertical-navigation-overlay');

        // Add a class depending on the transparentOverlay option
        if ( this.transparentOverlay )
        {
            this._overlay.classList.add('tropi-vertical-navigation-overlay-transparent');
        }
    ...
    private _showAsideOverlay(): void
    {
        ...
        this._asideOverlay.classList.add('tropi-vertical-navigation-aside-overlay');

    ...
    }) =>
    {
        ...
        addComponents(
            _.fromPairs(_.map(options.themes, (theme, themeName) => [
                ...
                ))), (value, key) => [[`--tropi-${e(key)}`, value], [`--tropi-${e(key)}-rgb`, chroma(value).rgb().join(',')]]))),
            ])),
        );
        ...
        const schemeCustomProps = _.map(['light', 'dark'], (colorScheme) =>
        {
            ...
            const background = theme(`tropi.customProps.background.${colorScheme}`);
            const foreground = theme(`tropi.customProps.foreground.${colorScheme}`);
            ...
            return {
                [(isDark ? darkSchemeSelectors : lightSchemeSelectors)]: {
                    ...
                    ..._.fromPairs(_.flatten(_.map(background, (value, key) => [[`--tropi-${e(key)}`, value], [`--tropi-${e(key)}-rgb`, chroma(value).rgb().join(',')]]))),
                    ..._.fromPairs(_.flatten(_.map(foreground, (value, key) => [[`--tropi-${e(key)}`, value], [`--tropi-${e(key)}-rgb`, chroma(value).rgb().join(',')]]))),
                },
            };
        });
            ...
            theme: {
                extend: {
                    ...
                    colors: _.fromPairs(_.flatten(_.map(_.keys(flattenColorPalette(normalizeTheme(options.themes.default))), (name) => [
                        [name, `rgba(var(--tropi-${name}-rgb), <alpha-value>)`],
                        [`on-${name}`, `rgba(var(--tropi-on-${name}-rgb), <alpha-value>)`],
                    ]))),
                },
                tropi  : {
                    customProps: {

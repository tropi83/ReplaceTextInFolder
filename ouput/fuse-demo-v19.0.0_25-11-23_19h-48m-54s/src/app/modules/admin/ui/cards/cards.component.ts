...
{
    @ViewChildren(FuseCardComponent, {read: ElementRef}) private _tropiCards: QueryList<ElementRef>;
    ...
    private _calcNumberOfCards(): void
    {
        ...
        this.filters.forEach((filter) =>
        {
            ...
            if ( filter === 'all' )
            {
                count = this._tropiCards.length;
            }
            else
            {
                count = this.numberOfCards[filter] = this._tropiCards.filter(tropiCard => tropiCard.nativeElement.classList.contains('filter-' + filter)).length;
            }
    ...
    {
        // Go through all tropi-cards
        this._tropiCards.forEach((tropiCard) =>
        {
            // If the 'all' filter is selected...
            if ( this.selectedFilter === 'all' )
            {
                // Remove hidden class from all cards
                tropiCard.nativeElement.classList.remove('hidden');
            }
            // Otherwise...
            else
            {
                // If the card has the class name that matches the selected filter...
                if ( tropiCard.nativeElement.classList.contains('filter-' + this.selectedFilter) )
                {
                ...
                else
                {
                    ...
                    tropiCard.nativeElement.classList.add('hidden');
                }
            }
        });
    }
